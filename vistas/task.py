    ...
    def post(self):
        ...
        if file:
            ...
            try:
                ...
                video = Video(status=TaskStatus.UPLOADED.value, raiting=raiting, id_user=user_id)
                db.session.add(video)
                db.session.commit()
                
                timestamp = datetime.now()
                task = Task(timestamp=timestamp, status=TaskStatus.UPLOADED.value, id_video=video.id, id_user=user_id)
                db.session.add(task)
                db.session.commit()
                ...
                file.save(file_path)
                response = {"video_id":video.id,"task_id":task.id,"user_id":user_id}
